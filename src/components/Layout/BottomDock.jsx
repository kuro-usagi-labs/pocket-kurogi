import { BarChart3, CalendarRange, Clock3, MessageCircle, PiggyBank, Settings2 } from 'lucide-react'

const navItems = [
  { id: 'chat', icon: MessageCircle, label: 'Chat' },
  { id: 'history', icon: Clock3, label: 'Histori' },
  { id: 'wallets', icon: PiggyBank, label: 'Dompet' },
  { id: 'planning', icon: CalendarRange, label: 'Rencana' },
...
export default function BottomDock({ activeTab, onTabChange }) {
  return (
    ...
    >
      <nav className="bottom-dock-surface pointer-events-auto grid h-[64px] w-full max-w-md grid-cols-6 gap-0.5 rounded-[20px] border border-midnight/[0.09] p-1.5 shadow-[0_18px_48px_-22px_rgba(22,24,28,0.42)] backdrop-blur-xl">
        {navItems.map((item) => {
